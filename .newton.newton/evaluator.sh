...
# Runs tests and returns 0 only if 100% tests pass. Saves both markdown and JSON for auditing.

TEST_JSON_FILE="./.newton/state/test_output.json"
TEST_MARKDOWN_FILE="./.newton/state/evaluator_status.md"

# Create state directory if it doesn't exist
mkdir -p "./.newton/state"

# Run tests using the dedicated test script (save both markdown and JSON for auditing)
echo "Running test evaluation..." >&2
./scripts/run-tests.sh -o "$TEST_MARKDOWN_FILE" -j "$TEST_JSON_FILE" >/dev/null 2>&1
